...
public class ScoreInsert2 {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < sList.size(); i++) {
			String hakbun = sList.get(i).getHakbun().substring(0,1)+"-"+sList.get(i).getHakbun().substring(1,2)+"-"+sList.get(i).getHakbun().substring(2,4);
			System.out.println(hakbun+"\t" + sList.get(i).getName() + "\t"
		...
		Score2_VO sco = manager.scoreList2();
		System.out.println("전체총점\t\t\t" + sco.getKorsum() + "\t" + sco.getEngsum() + "\t" + sco.getMatsum() + "\t"
				+ (sco.getKorsum() + sco.getEngsum() + sco.getMatsum()));
		System.out.println("전체평균\t\t\t" + sco.getKoravg() + "\t" + sco.getEngavg() + "\t" + sco.getMatavg() + "\t\t"
				+ Math.round((sco.getKoravg() + sco.getEngavg() + sco.getMatavg()) * 100. / 3) / 100.);
	}
}
